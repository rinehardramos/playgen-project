...
async function stageUploadAssets(scriptId: string, stationSlug: string, playlistDate: string): Promise<void> {
  ...
  for (const song of songs) {
    ...
    try {
      ...
      // Upload variants even for already-uploaded songs (#500 ABR)
      await uploadSongVariants(song.song_id, cdnUrl, s3, bucket, publicBase);
      continue;
    } catch { /* not found — transcode + upload */ }
...
// ── Stage: build_hls ────────────────────────────────────────────────────────
//
// Generates synchronised HLS streams for dynamic layered audio (#532, #500):
//
...
//
//   dj_low/mid/high   — Full-show silence track generated by ffmpeg (anullsrc) with every
//                       DJ audio clip mixed in at its exact program-timeline offset via
//                       ffmpeg adelay + amix. Three quality variants (32/128/256 kbps).
//
//   dj.m3u8           — Backward-compat alias pointing to the mid-quality DJ variant.
//
// hls_tracks JSONB shape after build_hls:
// { music: url, dj: url (mid-quality compat), dj_stream: url (ABR master), music_stream: url (ABR master) }
//
...
async function stageIngestProduction(scriptId: string, token: string): Promise<string> {
  ...
  const { rows: scriptRows } = await pool.query<{
    ...
    hls_tracks: { music: string; dj: string; dj_stream?: string; music_stream?: string } | null;
