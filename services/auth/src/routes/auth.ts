...
import {
  ...
  app.post('/auth/reset-password', {
    schema: { body: { type: 'object', required: ['token', 'password'], properties: { token: { type: 'string', minLength: 1 }, password: { type: 'string', minLength: 8 } } } },
  }, async (req, reply) => {
    ...
    try {
      ...
      if (err instanceof AuthError) return reply.code(400).send({ error: { code: err.code, message: err.message } });
      throw err;
    }
  });

  app.post('/auth/accept-invite', {
    schema: { body: { type: 'object', required: ['token', 'display_name', 'password'], properties: { token: { type: 'string', minLength: 1 }, display_name: { type: 'string', minLength: 1 }, password: { type: 'string', minLength: 8 } } } },
  }, async (req, reply) => {
    const { token, display_name, password } = req.body as { token: string; display_name: string; password: string };
    try {
